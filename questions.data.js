window.PREPME_QUESTIONS = {
  title: "Interview Prep — AI Agent 工程师 — XIAO",
  meta: {
    role: "AI Agent 工程师",
    candidate: "XIAO",
    language: "zh-CN",
    ...
    coverage: {
      ...
      cv: [
        "自研流程引擎（DAG+状态机+分片调度）",
        "RAG知识库（双轨检索+三层缓存+Bad Case治理）",
        "Agent能力层（ReAct+工具编排+Multi-Agent）",
        "MCP驱动的规范即代码",
        "水利平台项目",
        "API网关项目",
        "抽奖系统项目"
  ...
  promptTemplate: "你是一位资深技术面试官，正在面试一位有9年Java后端经验、最近2年转型AI Agent开发的候选人。请针对以下面试题进行深度分析：\n\n【面试题】{{QUESTION}}\n\n【可能的追问】\n{{FOLLOWUPS}}\n\n请提供：\n1. **实战回答**：一个有深度、结合项目实战的标准答案，像真人面试回答，不是论文\n2. **踩过的坑**：实际工程落地中遇到的问题和解决方案\n3. **加分项**：能让面试官眼前一亮的回答要点\n4. **追问应对**：针对上述追问，给出回答思路\n5. **架构图**：如果涉及架构或流程，请用ASCII图辅助说明\n\n请用中文回答，技术术语保留英文原文。",
  data: [
    {
    ...
    {
      category: "CV: Agent能力层",
      questions: [
        {
          q: "你在平台中设计了Agent能力层，支撑运维自愈和业务智能化两个场景。请介绍整体架构设计。",
          level: "Core",
          source: "CV: Agent能力层架构",
          followups: [
            "Agent的推理链路是如何设计的？",
            "工具编排的统一Schema规范是什么样的？",
            "为什么选择本地部署Qwen而不是用云端API？"
          ]
        },
        {
          q: "你提到使用了多Agent协同架构（Retriever/Reranker/Writer），这种设计解决了什么问题？",
          level: "Advanced",
          source: "CV: Multi-Agent协同",
          followups: [
            "多个Agent之间如何通信？",
            "如何处理Agent之间的状态同步？",
            "单Agent和多Agent架构如何选择？"
        ...
        {
          q: "你提到工具返回值做结构化预处理，避免撑爆context window。具体是怎么做的？",
          level: "Advanced",
          source: "CV: Agent工具编排优化",
          followups: [
            "日志聚合去重的策略是什么？",
            "指标趋势提取是如何实现的？",
            "如何平衡信息损失和context window限制？"
          ]
        }
      ]
    },
    {
      category: "CV: MCP与工程效能",
      questions: [
        {
          q: "你提到通过MCP协议打通YAPI接口规范与AI代码生成链路。请详细介绍这个方案。",
          level: "Core",
          source: "CV: MCP驱动的规范即代码",
          followups: [
            "MCP的Resource是如何定义的？",
            "AI生成的代码质量如何保障？",
            "这个方案的局限性是什么？"
