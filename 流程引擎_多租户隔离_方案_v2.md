...
```
shardCount = CPU 核心数 ~ CPU 核心数 × 2
```

| 场景 | 推荐分片数 | 说明 |
|------|----------|------|
| 4 核容器/虚拟机 | 4 ~ 8 | 每个 EventLoop 占用一个核心 |
| 8 核服务器 | 8 ~ 16 | 状态机计算轻量，可略超核心数 |
| 16 核以上 | 16 ~ 32 | 再往上收益递减，注意内存开销 |

**决策依据**：
- EventLoop 是轻量 CPU 密集型任务，一个分片可以跑满一个核心；
- 分片太少会导致多核利用率低、单分片饱和；
- 分片太多会增加内存开销（每个分片都有一份租户队列）和上下文切换成本。

**验证指标**：

```promql
# 各分片处理速率
sum(rate(flow_shard_processed_total[1m])) by (shard_id)

# 各分片队列深度
flow_shard_queue_depth{shard_id=~"0|1|2|3"}
```

| 现象 | 结论 | 行动 |
|------|------|------|
| 所有分片速率接近，CPU 用满 | 分片数合理 | 保持 |
| 部分分片饱和，其他空闲 | 热点实例或分片不均 | 检查 instance_id 分布，考虑 L3 动态分片 |
| 所有分片 CPU 未用完 | 分片数过多或流量不足 | 可适当减少 |
| 所有分片都饱和 | 机器 CPU 不足 | 扩容核心数 |

**变更注意事项**：
- `shardCount` 属于集群级配置，所有节点必须一致；
- 改变 `shardCount` 会改变 `instance_id` 路由目标，可能破坏实例级串行化；
- 变更需全集群同时重启，建议在配置中心统一管理。
...
public class TenantInstanceQuota {
    ...
    private static final String KEY = "flow:tenant:active_count";
    ...
    public boolean tryRegister(String tenantId) {
        ...
        // Redis 原子递增，多节点全局准确
        Long current = redisTemplate.opsForHash().increment(KEY, tenantId, 1);
        
        if (current == null) {
            // Redis 不可用，降级为 DB 计数
            current = flowInstanceRepo.countActiveByTenantId(tenantId);
        }
        ...
        if (current > hardLimit) {
            redisTemplate.opsForHash().increment(KEY, tenantId, -1);
...
public void reconcileActiveCount() {
    Map<String, Long> dbCounts = flowInstanceRepo.countActiveGroupByTenant();
    for (Map.Entry<String, Long> entry : dbCounts.entrySet()) {
        redisTemplate.opsForHash().put(KEY, entry.getKey(), String.valueOf(entry.getValue()));
    }
}
```
...
- 活跃实例计数使用 Redis 分布式哈希计数，保证多节点全局一致；
- Redis 不可用时降级为 DB 计数，定期以 DB 为准对账修正 Redis 计数；
...
> 以下估算基于 **2 个 4c8g Engine 节点 + AWS 最低配 Redis（cache.t3.micro）+ AWS 最低配 MySQL（db.t3.micro）** 的典型 POC/试用部署。

#### 1. 各组件基准能力

| 组件 | 规格 | 关键性能参数 | 对本方案的影响 |
|------|------|-------------|---------------|
| Engine | 2 × 4c8g | 共 8 vCPU / 16 GiB | 分片 EventLoop + Worker 线程池可轻松支撑数千 TPS，**不是瓶颈** |
| Redis | cache.t3.micro | 2 vCPU / 0.5 GiB / 10k+ ops/s | 限流 + 配额计数约 1~2 ops/请求，**不是瓶颈** |
| MySQL | db.t3.micro | 2 vCPU burstable / 1 GiB / gp2 20GB（基线 100 IOPS，突发 3000 IOPS） | **核心瓶颈** |

#### 2. 单次状态变更的 IO 开销

一次典型的状态推进（如 `TRANSITION` 或带回调的节点推进）涉及：

1. Redis 限流 Lua：1 次 RTT
2. 配额计数 `HINCRBY`：1 次 Redis 操作
3. DB 主键 `UPDATE flow_instance`：1 次磁盘写
4. 节点执行记录 `INSERT/UPDATE flow_node_execution`：1 次磁盘写（仅回调节点）

即：**每状态变更 ≈ 1~2 次 Redis ops + 1~2 次 DB 写**。

#### 3. TPS 估算

db.t3.micro 的 gp2 20GB 存储基线 IOPS 仅 100，突发上限 3000；且实例 CPU/内存极小，持续高负载下会很快耗尽 CPU credit。

| 场景 | 估算 TPS | 说明 |
|------|---------|------|
| 状态变更 TPS（持续） | **100 ~ 300** | 受 MySQL IOPS + CPU credit 限制；保守建议按 100 设计 |
| 状态变更 TPS（突发） | **500 ~ 1000** | 依赖 gp2 burst bucket，持续数分钟 ~ 十几分钟 |
| 实例创建 TPS | **100 ~ 300** | 与状态变更同量级，额外一次 INSERT flow_instance |
| 端到端流程完成 TPS | **20 ~ 60** | 假设平均每个流程 5 个节点，状态 TPS / 5 |

#### 4. 结论与建议

- **该最低配套餐能支撑的持续 TPS 约为 100~300 状态变更/秒**，远低于方案目标上限 5000 TPS。
- 瓶颈几乎完全在 **MySQL**，Redis 和 Engine 节点在该 TPS 下仍有大量余量。
- 若业务需要接近 5000 TPS，建议：
  - MySQL 升级到 **db.t4g.large / db.m6g.large** 或更高；
  - 存储改用 **gp3 并配置 3000+ IOPS**；
  - Redis 升级到 **cache.t4g.small / cache.t4g.medium**；
  - 此时 Engine 节点 2×4c8g 仍可能先成为 CPU 瓶颈，需同步压测验证。

> 注：以上数字为经验估算，实际生产环境必须做基线压测（见 7.1）后校准。
>
> 成本参考（us-east-1 On-Demand，MySQL/PostgreSQL）：db.m6g.large 约 **$0.15-0.18/小时**，Single-AZ 约 **$110-135/月**，Multi-AZ 约 **$220-270/月**；gp3 存储约 **$0.115/GB-月**（3000 IOPS 基线免费），超出基线的 IOPS 约 **$0.02/IOPS-月**。具体以 AWS 官网实时报价为准。
...
本方案的核心是"在共享服务内部建立租户级资源边界"，这一思想不仅适用于流程引擎，也适用于任何**多租户、异步任务型、有状态流转**的系统。推送系统是典型的复用场景。

### 12.1 推送系统与流程引擎的共性

| 流程引擎 | 推送系统 |
|---------|---------|
| 流程实例有生命周期状态 | 推送任务有 PENDING → SENT → DELIVERED → FAILED |
| 状态推进依赖下游回调 | 发送结果依赖 SMS/Push/Email 渠道商回调 |
| 多个租户共享引擎集群 | 多个 App/租户共享推送渠道资源 |
| 单个租户异常会扩散 | 某个 App 营销推送可能把渠道 API 打满 |
| 需要限流、隔离、熔断、重试 | 渠道 API 有 QPS/并发限制，失败要重试 |
...
流程引擎架构                    推送系统对应实现
─────────────────────────────────────────────────────────
入口层 Redis 限流          →    按 App/租户/渠道限流
调度层 ShardedFlowScheduler →    分片推送任务调度器
执行层 TieredFlowWorker    →    渠道发送执行器（大客户 VIP Lane，中小客户按套餐分组）
Behavior 熔断              →    渠道商 API 熔断保护
实例配额层 TenantInstanceQuota →    在推任务数上限 + 死任务归档
熔断兜底 TenantCircuitBreaker →    异常 App/租户熔断
```
...
#### 场景 1：多租户推送平台
- 多个 App 共用一套推送服务。
- 某 App 在双 11 进行大规模营销推送，瞬间占满短信渠道。
- **TieredFlowWorker 保证**：L 套餐大客户走 VIP Lane，S/M 套餐中小客户按套餐分组；A App 的突发流量不会塞满 B App 的队列，避免单租户异常扩散。

#### 场景 2：多渠道路由与降级
- 同一消息按优先级路由：SMS → App Push → Email。
- 每个渠道是一个 Behavior Handler，由 Resilience4j 熔断保护。
- 某渠道商故障时自动切换或降级，不影响整体推送能力。

#### 场景 3：批量营销推送
- 一次活动推送给 1000 万用户，分解为大量子任务。
- 公平轮询保证该租户不会垄断调度器，其他租户任务仍能被及时处理。

#### 场景 4：渠道回调状态流转
- 渠道商回调送达、点击、失败状态。
- SDK 自动接收回调并驱动推送任务状态机流转。
- 与流程引擎"业务方只写 Handler，SDK 自动回调"完全一致。

#### 场景 5：失败重试与死信
- 网络抖动导致推送失败，利用 `task_execution` 幂等表 + 重试表实现有状态重试。
- 超过重试次数进入死信队列，人工介入处理。

### 12.4 可复用的业务场景特征

只要业务系统符合以下 5 个特征，本架构就能复用：

1. **多租户共享资源**：存在单租户异常扩散风险。
2. **异步任务处理**：任务需要排队、调度，不是即时同步返回。
3. **有状态生命周期**：任务有明确的状态流转。
4. **依赖下游服务**：需要熔断、降级、重试机制。
5. **资源受限**：需要限流、配额保护。
...
因此，该架构不是为流程引擎单独设计的定制方案，而是一套可复用的 **SaaS 异步任务资源隔离基座**。
