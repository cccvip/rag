...
class McpGatewayTest {
    ...
    void setUp() throws IOException {
        ...
        List<ToolDefinition> tools = List.of(retrieve, restart);
        Map<String, String> services = Map.of(
                "rag-service", "http://localhost:" + mockPort,
                "ops-service", "http://localhost:" + mockPort
        );

        registry = new McpToolRegistry(tools, new DefaultTenantIsolation());
        StaticServiceResolver resolver = new StaticServiceResolver(services);
        gateway = new McpGateway(registry, resolver, new RestProtocolConverter(),
                new DefaultTenantIsolation(), 5000, new ObjectMapper());
    }
