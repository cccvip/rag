...
window.PREPME_ANSWERS["q5s1191"] = {
  ...
  date: "2025-06-17",
  answer: `<h2>实战回答</h2>
<p>那次事件的起因很简单：某个租户的 API Key 泄露了，攻击者用这个 Key 疯狂调用我们的 RAG 接口，一天之内产生了正常一个月的 LLM 费用。</p>

<h3>发现过程</h3>
<pre>
正常日均 LLM 费用：约 400 元
事件当天上午：费用突然飙到 1200 元（3 倍）
                   │
                   ▼
Grafana 告警：LLM Token 消耗异常
                   │
                   ▼
排查：发现某个租户的调用量是平时的 20 倍
                   │
                   ▼
确认：该租户 API Key 被盗用，攻击者在批量调用
</pre>
<p>发现靠的是<strong>成本监控告警</strong>。我们在 Grafana 上设置了阈值：单租户日消耗超过正常值 200% 时触发告警。这次事件后，我们又加了对比历史同期的异常检测（偏差超过 3σ 告警）。</p>

<h3>解决过程（分两步）</h3>
<p><strong>第一步：紧急止血</strong></p>
<ul>
<li>立即禁用被盗的 API Key</li>
<li>联系该租户确认情况，重新生成 Key</li>
<li>当天的异常费用不计入租户账单（我们的责任）</li>
</ul>

<p><strong>第二步：建立防御体系</strong></p>
<p>这次事件暴露了我们缺乏租户级的流量控制。之后我们做了三层防御：</p>
<pre>
┌─────────────────────────────────────────────┐
│              三层防御体系                     │
│                                             │
│  L1: 租户级 QPS 限流                         │
│      Redis 令牌桶，按套餐设置上限             │
│      基础版 100 QPS，企业版 500 QPS          │
│                                             │
│  L2: 三层缓存                                │
│      L1 答案缓存 + L2 HyDE 缓存 + L3 LLM 响应│
│      命中缓存的请求不消耗 Token              │
│                                             │
│  L3: 租户级 Token 配额                       │
│      每个租户有每日 Token 上限               │
...
<h2>踩过的坑</h2>
<ul>
<li><strong>告警太慢</strong>：一开始告警阈值设的是"日消耗超过 500%"，这次事件涨了 300% 才触发。后来调到 200%，并且加了小时级监控（不只是日级）。</li>
<li><strong>限流不能只看 QPS</strong>：攻击者可以降低 QPS 但增加单次请求的 Token 消耗（比如发很长的查询）。所以我们同时限 QPS 和每日 Token 总量。</li>
<li><strong>API Key 管理</strong>：之前 API Key 是明文存在租户的配置文件里，泄露风险高。后来改成：Key 只显示一次，之后只显示掩码；支持一键轮换；异常调用自动冻结。</li>
</ul>

<h2>加分项</h2>
<p>如果面试官追问"怎么防止类似事件再次发生"，可以说：</p>
<p>除了三层防御，我们还做了<strong>异常检测</strong>：</p>
<ul>
<li><strong>调用模式分析</strong>：正常用户的查询有明显的业务模式（比如工作时间集中、查询内容相关）。攻击者的调用模式通常是"高频 + 随机内容"。我们用简单的规则检测：如果 1 分钟内同一 Key 的查询内容相似度低于 0.3，标记为可疑。</li>
<li><strong>IP 维度限流</strong>：同一个 IP 对多个租户的 Key 发起请求，可能是攻击者在批量测试泄露的 Key。我们加了 IP 维度的限流。</li>
</ul>
<p>但说实话，<strong>100% 防住 API Key 泄露是不可能的</strong>。我们的策略是"预防 + 快速发现 + 快速止血"。预防靠 Key 管理，发现靠成本监控，止血靠自动冻结。</p>`,
  followups: [
    {
      q: "如何区分正常流量和攻击流量？",
      answer: `<p>我们用<strong>规则 + 统计</strong>结合的方式，不用 ML 模型（数据量不够）。</p>
<p><strong>规则检测</strong>：</p>
<ul>
<li>单 Key 1 分钟内 QPS > 正常值 5 倍 → 可疑</li>
<li>单 Key 查询内容相似度 < 0.3（随机查询）→ 可疑</li>
<li>单 IP 关联 > 3 个不同租户的 Key → 可疑（批量测试泄露 Key）</li>
<li>非工作时间（凌晨 2-6 点）调用量突然增加 10 倍 → 可疑</li>
</ul>
<p><strong>统计检测</strong>：</p>
<ul>
<li>对比历史同期数据，偏差超过 3σ → 告警</li>
<li>单租户 Token 消耗日环比增长 > 200% → 告警</li>
</ul>
<p>触发"可疑"后不会立即封禁，而是：</p>
<ol>
<li>降低该 Key 的 QPS 上限（从 100 降到 10）</li>
<li>通知租户确认是否是正常业务</li>
<li>如果 10 分钟内无响应，自动冻结 Key</li>
</ol>
<p>这个机制上线后，误报率大概 5%（正常租户大促期间被误判为可疑）。后来加了白名单——大促前租户可以申请临时提高阈值。</p>`
    },
    {
      q: "Token配额如何设计？",
      answer: `<p>我们的 Token 配额分三层：</p>
<ul>
<li><strong>租户级日配额</strong>：每个租户每天有 Token 上限，按套餐分：
<ul>
<li>基础版：50 万 Token/天</li>
<li>专业版：200 万 Token/天</li>
<li>企业版：不限量（但有 QPS 限制）</li>
</ul>
</li>
<li><strong>用户级日配额</strong>：单个用户每天的 Token 上限，防止某个用户消耗整个租户的配额</li>
<li><strong>单次请求上限</strong>：单次查询最多消耗 5000 Token，防止超长查询</li>
</ul>
<p>配额用 Redis 计数器实现，Key 是 <code>token_quota:{tenant_id}:{date}</code>，每次 LLM 调用后递增。</p>
<p>超限后的降级策略：</p>
<ul>
<li>日配额用完 → 降级为"缓存模式"（只返回缓存结果，不调 LLM）</li>
<li>单次超限 → 截断输入，提示用户"查询内容过长"</li>
</ul>
<p>踩过的坑：配额不能太紧。有个租户刚上线，导入了大量文档，前两天的查询量很大（文档测试），配额很快就用完了。后来加了"新租户首周配额翻倍"的规则。</p>`
    },
    {
      q: "如何防止租户之间的资源抢占？",
      answer: `<p>我们的资源隔离是<strong>五层体系</strong>，不只防 LLM 资源抢占，是全栈隔离：</p>
<ul>
<li><strong>入口限流</strong>：每个租户独立的 QPS 上限（Redis 令牌桶），超限直接拒绝</li>
<li><strong>分片调度</strong>：流程引擎按 instance_id 哈希分片，大租户独立 Lane，中小租户按套餐分组</li>
<li><strong>LLM 配额</strong>：每个租户独立的 Token 日配额，互不影响</li>
<li><strong>缓存隔离</strong>：缓存 Key 包含 tenant_id，A 租户的缓存不会被 B 租户命中</li>
<li><strong>租户熔断</strong>：某个租户的错误率超过阈值时，自动熔断，不影响其他租户</li>
</ul>
<p>核心思想：<strong>防止"吵闹邻居"</strong>。一个租户的流量洪峰不能影响其他租户。</p>
<p>我们实测过：单租户流量洪峰 10 倍时，其他租户的 P99 延迟变化 < 5%。这靠的是入口限流 + 分片调度的组合——限流控制总量，分片保证隔离。</p>
<p>对于 LLM 资源，最有效的手段是<strong>Token 配额</strong>。QPS 限流只能控制请求频率，不能控制单次请求的 Token 消耗。Token 配额直接控制总消耗量，即使攻击者降低 QPS 但增加单次 Token，也会被配额拦住。</p>`
    }
  ]
};
